...
test('all AI data dependencies exist in ordered migrations', async () => {
  const migrations=await Promise.all([
    ...
    '010_project_workflow_fields.sql','013_ai_providers.sql','014_project_serial_codes.sql','015_ai_usage.sql',
  ].map((name)=>readFile(new URL(`../migrations/${name}`,import.meta.url),'utf8')));
  const schema=migrations.join('\n');
  for (const token of ['CREATE TABLE IF NOT EXISTS projects','CREATE TABLE IF NOT EXISTS clients','CREATE TABLE IF NOT EXISTS tasks','CREATE TABLE IF NOT EXISTS professionals','CREATE TABLE IF NOT EXISTS equipment_catalog','CREATE TABLE IF NOT EXISTS project_equipment','CREATE TABLE IF NOT EXISTS ai_provider_settings','CREATE TABLE IF NOT EXISTS ai_usage_log','priority_customer_number','assignee_professional_id','serial_code','parent_id','item_type','quantity','company_name','job_title','affiliation','project_size','contractor_progress']) {
