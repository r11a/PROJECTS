...
- נוספה שמירה מרכזית במסד הנתונים לסדר, צבע, שם קבוצה, תיוג, צבע שורה, ערכי עמודות מותאמות וכמות מותקנת.
- נוספה עריכה ישירה של שם וצבע מערכת, מיקום, תיוג, כמות, כמות מותקנת, סטטוס ושדות מותאמים.
- סיכומי ITEM מתעדכנים אוטומטית מנתוני ה־SUBITEMS: כמות, הותקן ויתרה.
- הגרירה הוגבלה לידיות ייעודיות כדי לא להפריע לעריכת תאים, עם פוקוס ברור, גלילה אופקית איכותית ותצוגת Mobile ו־Dark Mode.
- הוספה, שכפול, מחיקה, קיבוץ ועמודות מותאמות מסונכרנים כעת בין כל משתמשי הפרויקט.

## 0.31.1

- חלון יצירת משימה נסגר כעת מיד עם שליחת הטופס, בזמן שהשמירה והפידבק ממשיכים ברקע.
- לוח המערכות ההיררכי הוטמע ישירות בתוך כל פרויקט והופרד לחלוטין מטאב צוות הפרויקט.
- נוספו בפרויקט שורות ITEM מתקפלות, טבלאות SUBITEM, חישובי כמות/התקנה/יתרה, סטטוסים צבעוניים, עריכה ישירה, שכפול, מחיקה ועמודות נגררות.
- נתוני ההתקנה נשמרים כעת במסד הנתונים ומעדכנים מיידית את סיכומי שורת המערכת.

## 0.31.0

- נבנה מחדש מסך מערכות ורכיבים כלוח היררכי בסגנון Monday: קטגוריות, פריטי אב, תתי־פריטים, עריכה ישירה וסיכומי כמות/התקנה/יתרה בזמן אמת.
- נוספו מיקום, תיוג, מק״ט, כמות וסטטוס צבעוני לכל רכיב, לצד הוספה, שכפול, מחיקה ועמודות אישיות נגררות.
- הופרדו מערכות ורכיבים מצוות הפרויקט לשני אזורי עבודה עצמאיים.
- תוקן תהליך יצירת משימה: פידבק שמירה, חסימת לחיצה כפולה וסגירת החלון רק לאחר הצלחה.
- שעת התחלת משימה מוצגת בפורמט 24 שעות ובטווח העבודה 08:00–18:00.
- משימות שהושלמו מופרדות לטבלה תחתונה בפרויקט ומוסרות אוטומטית מלוחות הגאנט.
- תוקן רענון מאגר אנשי המקצוע לאחר יצירה ושיוך ישירות מתוך פרויקט, כך שהתפקיד זמין מיד גם בפרויקטים אחרים.

## 0.30.1

- תוקן מנוי Outlook שנוצר בטעות מכתובת Home Assistant Ingress וגרם לבקשות חוזרות לשם משתמש וסיסמה.
- מסך Outlook דורש כעת כתובת Web ישירה של PROJECTS, חוסם כתובות Ingress ומפיק קישור ICS ציבורי ייעודי ללא credentials.
- נוספה אזהרה ברורה והנחיה להחלפת המנוי הישן ב־Outlook; יצירת קישור חדש מבטלת את הטוקן הקודם.
- חודש אייקון המיתוג ללא המסגרת השחורה ונוספו נכסים נפרדים לדפדפן, Apple Touch, PWA maskable והתראות Push.
- עודכן מטמון ה־Service Worker כדי להפיץ את האייקונים החדשים גם להתקנות PWA קיימות.

## 0.30.0

- נוסף מצב Offline מלא ל־PWA עם טעינת מעטפת האפליקציה והמידע האחרון שנצפה גם ללא רשת.
- נוסף Outbox מקומי מבוסס IndexedDB למשימות, דיווחי שעות, ביקורות, פגישות, טפסים, הודעות, עדכונים, תמונות, מסמכים והקלטות קוליות.
- נוסף סנכרון אוטומטי בחזרת החיבור, סנכרון ידני, מונה פעולות, סטטוס כשל וניסיון חוזר.
- נוספה התאמת מזהים זמניים כך שקבצים ומשימות המשך נשארים משויכים לרשומת האב לאחר הסנכרון.
- נוסף מנגנון Idempotency מאומת בצד השרת שמונע יצירה כפולה כאשר החיבור נקטע לאחר שהשרת כבר שמר פעולה.
- פעולות מחיקה, הרשאות וכספים אינן נכנסות אוטומטית לתור Offline כדי למנוע שינוי רגיש או הרסני ללא הקשר עדכני.
- נוסף חיווי מחובר, Offline ומסתנכרן בכותרת ומסך ניהול מפורט של תור הסנכרון.

## 0.29.3

- הודעות Push למשימות נשלחות אוטומטית ביצירה, בשינוי שיוך ובמועד היעד לכל המבצעים, האחראי והשיוך הישיר למשתמש.
- הודעות פנימיות מפעילות Push אוטומטי עם שם השולח, הנושא ותוכן ממוקד, וקישור חוזר למסך הרלוונטי.
- כשל Push אינו מבטל עוד משימה או הודעה שכבר נשמרו; ממשק המשתמש מציג בנפרד אישור שמירה ותוצאת מסירת ההתראה.
- נוסף פידבק שמירה ברור וארוך יותר עם אפשרות סגירה ידנית.
- תמונות ומסמכים מקושרים לביקורות אתר ולסיכומי פגישות ומוצגים מתוך הרשומה עצמה, כולל קישור בטוח לקבצים היסטוריים.
- דיווחי שעות ניתנים כעת לעריכה ולמחיקה, ובנק השעות מסתנכרן עם ביקורות, פגישות וטפסים גם בעדכון ובמחיקה.
- תוקן טיפול בתאריכים מסוג date-only כדי למנוע ירידה של יום בעריכת דיווח שעות.

## 0.29.2

- תוקן `BadJwtToken` ב־Apple Web Push באמצעות VAPID subject ציבורי ותקין במקום דומיין מקומי שנדחה ב־iOS.
- מפתחות ה־VAPID הקיימים נשמרים ולכן אין צורך ברישום מחדש של מכשירים תקינים.
- שגיאות ספקי Push מוצגות כעת בעברית עם הנחיית תיקון ברורה במקום JSON גולמי.

## 0.29.1

- נוסף כפתור בדיקה יזומה שעובר במסלול Web Push המלא ומחזיר תוצאת מסירה מיידית.
- נוסף פידבק מפורט: נמסרו, נכשלו, דולגו, מסירה אחרונה וסיבת כשל לפי מכשיר.
- שליחה מיידית ממתינה לתוצאת ספק ה־Push ולא מוצגת עוד כהצלחה לפני שהתקבלה תוצאה.
- נוספו שעות שקט אישיות וארגוניות; ההתראות ממשיכות להימסר ללא צליל או רטט.
- תוקן סטטוס שליחה כאשר אין מנוי מכשיר פעיל או כשההתראה חסומה בהעדפות.

## 0.29.0

- נוסף Web Push מלא ל־PWA באייפון ובאנדרואיד, כולל קבלת התראות כשהאפליקציה ברקע או המסך כבוי.
- נוסף מרכז התראות ב־ADMIN עם מדיניות מערכת, קטגוריות, משתמשים בודדים, רשימות תפוצה ונמעני פרויקט רלוונטיים.
- נוספו הודעות מיידיות ומתוזמנות, היסטוריית מסירה, ביטול תזמון ומנגנון מניעת כפילויות.
- נוספו התראות חכמות למשימות באיחור, גבייה שהגיעה למועד ופרויקטים בסיכון.
- נוספה התאמה אישית לכל נמען, כותרות ממוקדות וזיהוי ויזואלי באמצעות לוגו ללא מלל מיותר.
- נשמרת פרטיות כספית בצד השרת ומנויי מכשירים שפג תוקפם מושבתים אוטומטית.
