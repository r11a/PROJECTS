...
export async function createPushService({pool,authenticate,requireRoles,audit}) {
  ...
  const notifyTask=async(taskId,eventType,actor={})=>{const result=await pool.query(`SELECT t.id,t.title,t.description,t.due_date,t.start_time,t.end_time,t.project_id,p.name project_name,creator.display_name creator_name,
    ARRAY(SELECT DISTINCT target.user_id FROM (SELECT t.assignee_id user_id UNION SELECT professional.linked_user_id FROM professionals professional WHERE professional.id IN (t.assignee_professional_id,t.owner_professional_id) UNION SELECT professional.linked_user_id FROM task_assignees assignment JOIN professionals professional ON professional.id=assignment.professional_id WHERE assignment.task_id=t.id) target WHERE target.user_id IS NOT NULL) user_ids
    FROM tasks t LEFT JOIN projects p ON p.id=t.project_id LEFT JOIN users creator ON creator.id=t.created_by WHERE t.id=$1`,[taskId]);if(!result.rowCount)return {sent:0,failed:0,skipped:0};const task=result.rows[0],actorName=actor.displayName||actor.display_name||actor.username||task.creator_name||'משתמש';const title=eventType==='created'?'משימה חדשה שובצה עבורך':'משימה ששויכה אליך עודכנה';const schedule=[task.due_date,task.end_time||task.start_time].filter(Boolean).join(' · ');return sendToUsers(task.user_ids||[],{title,body:`${actorName} · ${task.project_name||'ללא פרויקט'} · ${task.title}${schedule?` · יעד ${schedule}`:''}`,url:`?page=tasks&task=${task.id}`,tag:`task-${eventType}-${task.id}`},{category:'tasks',dedupeKey:`task-${eventType}:${task.id}:${eventType==='created'?'initial':Date.now()}`});};
  const processSmart=async()=>{
    const current=await policy();if(!current.enabled)return;
    if(current.smart.overdueTasks){
      const dueNow=await pool.query(`SELECT t.id,t.title,t.due_date,t.end_time,t.start_time,p.name project_name,creator.display_name creator_name,ARRAY(SELECT DISTINCT target.user_id FROM (SELECT t.assignee_id user_id UNION SELECT professional.linked_user_id FROM professionals professional WHERE professional.id IN (t.assignee_professional_id,t.owner_professional_id) UNION SELECT professional.linked_user_id FROM task_assignees assignment JOIN professionals professional ON professional.id=assignment.professional_id WHERE assignment.task_id=t.id) target WHERE target.user_id IS NOT NULL) user_ids FROM tasks t JOIN projects p ON p.id=t.project_id LEFT JOIN users creator ON creator.id=t.created_by WHERE t.status NOT IN ('done','completed','cancelled') AND (t.due_date::timestamp+COALESCE(t.end_time,t.start_time,'09:00'::time))<=(CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Jerusalem') AND t.due_date>=CURRENT_DATE-1 LIMIT 100`);
      for(const task of dueNow.rows)await sendToUsers(task.user_ids||[],{title:'הגיע מועד המשימה',body:`${task.creator_name||'משתמש'} · ${task.project_name} · ${task.title}`,url:`?page=tasks&task=${task.id}`,tag:`task-due-${task.id}`},{category:'tasks',dedupeKey:`task-due:${task.id}:${task.due_date}:${task.end_time||task.start_time||'09:00'}`});
      const tasks=await pool.query(`SELECT t.id,t.title,t.due_date,p.name project_name,ARRAY(SELECT DISTINCT target.user_id FROM (SELECT t.assignee_id user_id UNION SELECT professional.linked_user_id FROM professionals professional WHERE professional.id IN (t.assignee_professional_id,t.owner_professional_id) UNION SELECT professional.linked_user_id FROM task_assignees assignment JOIN professionals professional ON professional.id=assignment.professional_id WHERE assignment.task_id=t.id) target WHERE target.user_id IS NOT NULL) user_ids FROM tasks t JOIN projects p ON p.id=t.project_id WHERE t.status NOT IN ('done','completed','cancelled') AND t.due_date<CURRENT_DATE LIMIT 100`);
  ...
  return {router,publicKey:keys.publicKey,sendToUsers,notifyTask,processCampaigns,processSmart};
}
