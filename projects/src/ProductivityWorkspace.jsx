...
const taskTiming=(task)=>{const today=new Date();today.setHours(0,0,0,0);const startValue=dateOnly(task.start_date||task.startDate);const dueValue=dateOnly(task.due_date||task.dueDate);const start=startValue?new Date(`${startValue}T00:00:00`):null;const days=start?Math.ceil((start-today)/86400000):null;const countdown=days>1?`מתחילה בעוד ${days} ימים`:days===1?'מתחילה מחר':days===0?'מתחילה היום':days<0?'כבר התחילה':'';return [`התחלה: ${dayText(startValue)}`,`סיום: ${dayText(dueValue)}`,countdown].filter(Boolean).join(' · ')};
const calendarDate=(year,month,day)=>new Date(year,month,Math.min(day,new Date(year,month+1,0).getDate()));
const projectAge=(value,now=new Date())=>{const started=new Date(value);if(!value||Number.isNaN(started.getTime()))return null;const start=calendarDate(started.getFullYear(),started.getMonth(),started.getDate());const end=calendarDate(now.getFullYear(),now.getMonth(),now.getDate());if(start>end)return {years:0,months:0,days:0};let years=end.getFullYear()-start.getFullYear();let yearAnchor=calendarDate(start.getFullYear()+years,start.getMonth(),start.getDate());if(yearAnchor>end){years-=1;yearAnchor=calendarDate(start.getFullYear()+years,start.getMonth(),start.getDate())}let months=(end.getFullYear()-yearAnchor.getFullYear())*12+end.getMonth()-yearAnchor.getMonth();let monthAnchor=calendarDate(yearAnchor.getFullYear(),yearAnchor.getMonth()+months,yearAnchor.getDate());if(monthAnchor>end){months-=1;monthAnchor=calendarDate(yearAnchor.getFullYear(),yearAnchor.getMonth()+months,yearAnchor.getDate())}const days=Math.round((Date.UTC(end.getFullYear(),end.getMonth(),end.getDate())-Date.UTC(monthAnchor.getFullYear(),monthAnchor.getMonth(),monthAnchor.getDate()))/86400000);return {years,months,days}};
const projectAgeText=(value)=>{const age=projectAge(value);if(!age)return 'גיל הפרויקט לא זמין';const unit=(value,one,many)=>`${value} ${value===1?one:many}`;return `גיל הפרויקט: ${unit(age.years,'שנה','שנים')} · ${unit(age.months,'חודש','חודשים')} · ${unit(age.days,'יום','ימים')}`};
...
export function MyWorkWorkspace({api,user,projects,professionals,setNotice,openProject}){
  ...
  const saveTask=async(value)=>{try{const result=await api(`/operations/tasks/${editor.id}`,{method:"PATCH",body:JSON.stringify(value)});setEditor(null);setNotice(result.offlineQueued?"נשמר במכשיר · ממתין לסנכרון":"המשימה עודכנה");load({silent:true});return true}catch(error){setNotice(error.message);return false}};
  ...
  const nextTask = [...Object.values(data.sections || {}).flat()].sort((a,b) => {
    const rank = task => (task.critical ? 4 : 0) + (dateOnly(task.due_date) && dateOnly(task.due_date) < localDateValue() ? 3 : 0) + (task.priority === 'urgent' ? 2 : task.priority === 'high' ? 1 : 0);
    return rank(b)-rank(a) || String(a.due_date||'9999').localeCompare(String(b.due_date||'9999'));
  })[0];
