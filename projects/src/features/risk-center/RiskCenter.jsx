...
export function RiskCenter({api,projects,openProject}){
  const [risks,setRisks]=useState([]);const [category,setCategory]=useState('all');
  ...
  return <section className="panel risk-center"><header><div><h3>מרכז סיכונים</h3><p>ציון בריאות מחושב ממשימות, גאנט, שעות, גבייה, ציוד והתקדמות קבלן</p></div><span>{visible.length} דורשים תשומת לב</span></header><nav className="project-category-filter"><button className={category==='all'?'active':''} onClick={()=>setCategory('all')}>הכל</button><button className={category==='smart_home'?'active':''} onClick={()=>setCategory('smart_home')}>בית חכם</button><button className={category==='other'?'active':''} onClick={()=>setCategory('other')}>אחרים</button></nav>{visible.length?visible.map((risk)=>{const project=projects.find((item)=>String(item.id)===String(risk.id));const categoryName=risk.project_category==='other'?(risk.project_category_custom||'אחר'):'בית חכם';return <button key={risk.id} onClick={()=>openRisk(risk,project)}><span className={`health-score ${risk.tone}`}>{risk.score}</span><div><strong>{risk.name}<small> · {categoryName}</small></strong><ul>{risk.reasons.slice(0,3).map((reason,index)=><li key={`${reason.kind}-${index}`}><CircleAlert size={13}/>{reason.label}<b>-{reason.points}</b></li>)}</ul></div><ChevronLeft/></button>}):<div className="risk-clear"><ShieldCheck/><strong>אין פרויקטים בסיכון בסיווג שנבחר</strong><span>לא זוהו כרגע גורמי סיכון פעילים.</span></div>}</section>;
}
