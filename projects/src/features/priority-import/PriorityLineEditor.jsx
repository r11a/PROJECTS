...
export function PriorityLineEditor({ line, index, systems, catalogItems, canCreateCatalogItems, onChange }) {
  ...
  return (
    <article className={`priority-line ${line.include ? "included" : "excluded"}`}>
      ...
      <label data-label="מערכת יעד"><select value={line.projectSystemId || ""} onChange={(event) => patch({ projectSystemId: event.target.value, systemManuallyChanged:true })}><option value="">בחירת מערכת…</option>{systems.map((system) => <option value={system.id} key={system.id}>{system.name}</option>)}</select><small>{line.mappingSource==='manual'?'נלמד ידנית':line.mappingSource==='catalog'?'זוהה מהקטלוג':'זוהה לפי תוכן'}</small></label>
