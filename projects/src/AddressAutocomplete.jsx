...
export function AddressAutocomplete({ api, value, onChange, onSelect, label = 'כתובת', required = false, className = '' }) {
  ...
  return <label className={`address-autocomplete ${className}`}>
    ...
    {open && items.length > 0 && <div className="address-suggestions">{items.map((item) => <button type="button" key={item.placeId} onMouseDown={(event) => event.preventDefault()} onClick={() => choose(item)}><MapPin size={14}/><span>{item.address}</span></button>)}</div>}
  </label>;
}
