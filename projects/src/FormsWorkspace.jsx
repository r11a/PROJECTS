...
export function FormsWorkspace({ api, apiRoot, user, setNotice }) {
  ...
  const uploadFile = async (file, target) => {
    ...
    body.set('file',file);
    setUploading(true);
    try{const result=await api('/documents',{method:'POST',body});setNotice(result.offlineQueued?'✓ הקובץ נשמר במכשיר ויועלה אוטומטית בחזרת החיבור':file.type?.startsWith('image/')?'התמונה הועלתה בהצלחה':'הקובץ הועלה בהצלחה');if(!result.offlineQueued)await load();}
...
function FilesHub({ files, clients, projects, apiRoot, canUpload, uploading, onUpload }) {
  ...
  return <div className="forms-files-hub">{canUpload&&<section className="panel forms-capture-bar"><div><span><Camera size={21}/></span><div><h3>צילום והעלאת קבצים</h3><p>צלמו מהטלפון או בחרו תמונה, PDF ומסמך מהמכשיר.</p></div></div><label>שיוך<select value={target} onChange={(event)=>setTarget(event.target.value)}><option value="">בחירת לקוח או פרויקט</option><optgroup label="פרויקטים">{projects.map((item)=><option key={item.id} value={`project:${item.id}`}>{item.name}</option>)}</optgroup><optgroup label="לקוחות">{clients.map((item)=><option key={item.id} value={`client:${item.id}`}>{item.name}</option>)}</optgroup></select></label><div className="capture-actions"><label className={uploading?'disabled':''}><Camera size={17}/>צילום עכשיו<input type="file" accept="image/*" capture="environment" disabled={uploading} onChange={(event)=>{const file=event.target.files?.[0];if(file)onUpload(file,target);event.target.value='';}}/></label><label className={uploading?'disabled':''}><Upload size={17}/>{uploading?'מעלה...':'בחירה מהמכשיר'}<input type="file" accept="image/*,video/*,application/pdf,.doc,.docx,.xls,.xlsx" disabled={uploading} onChange={(event)=>{const file=event.target.files?.[0];if(file)onUpload(file,target);event.target.value='';}}/></label></div></section>}{files.length?<section className="panel operational-files-list">{files.map((file,index) => <button type="button" key={file.id} className="operational-file-row" onClick={()=>setPreviewIndex(index)}><span className={file.mime_type?.startsWith('image/')?'file-thumbnail':''}>{file.mime_type?.startsWith('image/')?<img src={`${apiRoot}/documents/${file.id}/preview`} alt="" loading="lazy"/>:file.mime_type?.startsWith('video/')?<Film size={19}/>:<FileText size={19} />}</span><div><strong>{file.title||file.original_name}</strong><small>{file.project_name||file.client_name||'ללא שיוך'} · {new Date(file.created_at).toLocaleDateString('he-IL')} · {file.uploaded_by_name||'מערכת'}</small></div><em>{formatSize(file.size_bytes)}</em><EyeAction/></button>)}</section>:<Empty text="עדיין לא הועלו קבצים או תמונות." />}{previewFile&&<DocumentViewer file={previewFile} apiRoot={apiRoot} position={previewIndex} total={files.length} onClose={()=>setPreviewIndex(-1)} onPrevious={()=>setPreviewIndex(index=>(index-1+files.length)%files.length)} onNext={()=>setPreviewIndex(index=>(index+1)%files.length)}/>}</div>;
}

function EyeAction(){return <span className="file-view-action"><ExternalLink size={17}/><small>צפייה</small></span>}

function DocumentViewer({file,apiRoot,position,total,onClose,onPrevious,onNext}){
  const [zoom,setZoom]=useState(1);
  useEffect(()=>setZoom(1),[file.id]);
  const previewUrl=`${apiRoot}/documents/${file.id}/preview`;
  const downloadUrl=`${apiRoot}/documents/${file.id}/download`;
  const isImage=file.mime_type?.startsWith('image/');
  const isVideo=file.mime_type?.startsWith('video/');
  const isPdf=file.mime_type==='application/pdf';
  return <AppModal title={file.title||file.original_name} subtitle={`${file.project_name||file.client_name||'ללא שיוך'} · ${position+1} מתוך ${total}`} onClose={onClose} className="document-viewer-modal">
    <div className="document-viewer" onDoubleClick={()=>isImage&&setZoom(value=>value===1?2:1)}>
      <div className="document-viewer-toolbar"><div>{isImage&&<><button type="button" onClick={()=>setZoom(value=>Math.max(.5,value-.25))} title="הקטנה"><Minus size={17}/></button><b>{Math.round(zoom*100)}%</b><button type="button" onClick={()=>setZoom(value=>Math.min(4,value+.25))} title="הגדלה"><Plus size={17}/></button><button type="button" onClick={()=>setZoom(1)} title="איפוס"><RotateCcw size={16}/></button></>}</div><div><button type="button" onClick={onPrevious} disabled={total<2} title="הקודם"><ChevronRight size={19}/></button><button type="button" onClick={onNext} disabled={total<2} title="הבא"><ChevronLeft size={19}/></button></div></div>
      <div className={`document-viewer-stage ${isImage?'image-stage':''}`}>{isImage?<img src={previewUrl} alt={file.title||file.original_name} style={{width:`${zoom*100}%`,height:'auto'}}/>:isVideo?<video src={previewUrl} controls playsInline preload="metadata"/>:isPdf?<iframe src={`${previewUrl}#view=FitH`} title={file.title||file.original_name}/>:<div className="document-viewer-unsupported"><FileText size={58}/><h3>לא ניתן להציג קובץ זה ישירות בדפדפן</h3><p>קובצי Word ו־Excel ייפתחו באמצעות האפליקציה המתאימה במכשיר.</p><a href={previewUrl} target="_blank" rel="noreferrer"><ExternalLink size={16}/>פתיחה באמצעות המכשיר</a></div>}</div>
      <footer className="document-viewer-footer"><div><strong>{file.original_name}</strong><small>{new Date(file.created_at).toLocaleString('he-IL')} · {file.uploaded_by_name||'מערכת'} · {formatSize(file.size_bytes)}</small></div><a href={downloadUrl}><Download size={17}/>הורדה למכשיר</a></footer>
    </div>
  </AppModal>
}
