...
function RecordEditor({ initial, initialTemplate, templates, clients, projects, api, user, onClose, onSaved, onDelete, setNotice }) {
  ...
  const [form, setForm] = useState({ title: initial?.title || initialTemplate?.name || templates[0]?.name || '', clientId: initial?.clientId || '', projectId: initial?.projectId || '', scheduledFor: initial?.scheduledFor?.slice?.(0, 10) || '', notes: initial?.notes || '', values: initial?.values || {} });
  const [saving, setSaving] = useState(false);
