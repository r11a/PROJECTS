    ...
    {!groups.length?<div className="inline-empty">טרם שויכו מערכות ורכיבים לפרויקט.</div>:<div className="project-systems-scroll"><div className="project-systems-table" style={{'--project-custom-columns':columns.length}}>
      ...
      {groups.map(group=>{const total=group.items.reduce((sum,item)=>sum+Number(item.quantity_ordered??item.quantity??0),0);const installed=group.items.reduce((sum,item)=>sum+Number(item.quantity_installed||0),0);const remaining=Math.max(0,total-installed);const key=group.id||group.name;const open=expanded.has(key);return <article onDragOver={event=>event.preventDefault()} onDrop={()=>moveGroup(group.id)} className={`project-system-item ${open?'is-open':''}`} key={key} style={{'--system-color':group.color}}>
        ...
        {open&&<div className="project-subitems">
          <div className="project-subitem-head"><span>רכיב</span><span>מיקום</span><span>תיוג</span><span>מק״ט</span><span>כמות</span><span>הותקן</span><span>סטטוס</span>{columns.map((column,index)=><span key={column.column_key} draggable onDragStart={event=>{event.stopPropagation();event.dataTransfer.setData('text/plain',String(index))}} onDragOver={event=>event.preventDefault()} onDrop={event=>{event.stopPropagation();moveColumn(Number(event.dataTransfer.getData('text/plain')),index)}}><GripVertical size={12}/><b>{column.label}</b>{canManage&&<button type="button" title="מחיקת עמודה" onClick={async()=>{await api(`/projects/${projectId}/system-columns/${column.id}`,{method:'DELETE'});onReload()}}>×</button>}</span>)}<span>פעולות</span></div>
          {group.items.map(item=><div onDragOver={event=>event.preventDefault()} onDrop={event=>{event.stopPropagation();moveRow(group,item.id)}} className="project-subitem-row" key={item.id} style={{'--row-color':item.row_color||group.color}}>
