...
function ProjectSystemsBoard({items,columns,canEdit,canManage,user,api,projectId,onAdd,onReload,onDelete,setNotice}){
  ...
  {!groups.length?<div className="inline-empty">טרם שויכו מערכות ורכיבים לפרויקט.</div>:<div className="project-systems-scroll"><div className="project-systems-table" style={{'--project-custom-columns':columns.length}}><div className="project-system-head"><span>ITEM</span><span>סוג מערכת</span><span>סה״כ כמות</span><span>הותקן</span><span>יתרה</span><span/></div>{groups.map(group=>{const total=group.items.reduce((sum,item)=>sum+Number(item.quantity_ordered??item.quantity??0),0);const installed=group.items.reduce((sum,item)=>sum+Number(item.quantity_installed||0),0);const open=expanded.has(group.id||group.name);return <div onDragOver={event=>event.preventDefault()} onDrop={()=>moveGroup(group.id)} className="project-system-item" key={group.id||group.name} style={{'--system-color':group.color}}><div className="project-system-row"><button onClick={()=>setExpanded(current=>{const next=new Set(current),key=group.id||group.name;next.has(key)?next.delete(key):next.add(key);return next})}>{open?<ChevronDown size={17}/>:<ChevronLeft size={17}/>}</button><span className="system-title-cell"><i draggable={canEdit&&Boolean(group.id)} onDragStart={()=>setDraggedGroup(group.id)} title="גרירת המערכת"><GripVertical size={15}/></i><input disabled={!canManage||!group.id} defaultValue={group.name} onBlur={event=>group.id&&api(`/projects/${projectId}/system-board/${group.id}`,{method:'PATCH',body:JSON.stringify({title:event.target.value,color:group.color,sortOrder:group.sort})}).then(onReload)}/></span><span>{group.type}</span><b>{total}</b><b className="installed">{installed}</b><b className="remaining">{Math.max(0,total-installed)}</b><input className="system-color-picker" disabled={!canManage||!group.id} type="color" value={group.color} onChange={event=>group.id&&api(`/projects/${projectId}/system-board/${group.id}`,{method:'PATCH',body:JSON.stringify({title:group.name,color:event.target.value,sortOrder:group.sort})}).then(onReload)}/></div>{open&&<div className="project-subitems"><div className="project-subitem-head"><span>פריט</span><span>מיקום</span><span>תיוג</span><span>מק״ט</span><span>כמות</span><span>הותקן</span><span>סטטוס</span>{columns.map((column,index)=><span key={column.column_key} draggable onDragStart={event=>{event.stopPropagation();event.dataTransfer.setData('text/plain',String(index))}} onDragOver={event=>event.preventDefault()} onDrop={event=>{event.stopPropagation();moveColumn(Number(event.dataTransfer.getData('text/plain')),index)}}><GripVertical size={12}/>{column.label}{canManage&&<button onClick={async()=>{await api(`/projects/${projectId}/system-columns/${column.id}`,{method:'DELETE'});onReload()}}>×</button>}</span>)}<span/></div>{group.items.map(item=><div onDragOver={event=>event.preventDefault()} onDrop={event=>{event.stopPropagation();moveRow(group,item.id)}} className="project-subitem-row" key={item.id} style={{'--row-color':item.row_color||'transparent'}}><span className="subitem-name"><i draggable={canEdit} onDragStart={event=>{event.stopPropagation();setDraggedRow(item.id)}} title="גרירת השורה"><GripVertical size={14}/></i><strong>{item.name}</strong></span><input disabled={!canEdit} defaultValue={item.location||''} onBlur={event=>save(item,{location:event.target.value})}/><input disabled={!canEdit} defaultValue={item.tag||''} onBlur={event=>save(item,{tag:event.target.value})}/><span>{item.priority_sku||item.code||'—'}</span><input disabled={!canEdit} type="number" min="0" step="1" defaultValue={Number(item.quantity_ordered??item.quantity??0)} onBlur={event=>save(item,{quantity:Number(event.target.value)})}/><input disabled={!canEdit} type="number" min="0" step="1" max={Number(item.quantity_ordered??item.quantity??0)} defaultValue={Number(item.quantity_installed||0)} onBlur={event=>save(item,{quantityInstalled:Number(event.target.value),status:Number(event.target.value)>=Number(item.quantity_ordered??item.quantity??0)?'installed':item.status})}/><select disabled={!canEdit} className={`monday-status ${item.status||'waiting'}`} value={item.status==='planned'?'waiting':item.status||'waiting'} onChange={event=>save(item,{status:event.target.value,quantityInstalled:event.target.value==='installed'?Number(item.quantity_ordered??item.quantity??0):Number(item.quantity_installed||0)})}><option value="waiting">ממתין</option><option value="in_progress">בביצוע</option><option value="installed">הותקן</option></select>{columns.map(column=><input disabled={!canEdit} key={column.column_key} defaultValue={item.custom_values?.[column.column_key]||''} onBlur={event=>save(item,{customValues:{...(item.custom_values||{}),[column.column_key]:event.target.value}})}/>)}<span className="project-subitem-actions"><input className="row-color-picker" disabled={!canEdit} type="color" value={item.row_color||'#ffffff'} onChange={event=>save(item,{rowColor:event.target.value})}/>{canEdit&&<button title="שכפול" onClick={async()=>{try{await api(`/projects/${projectId}/equipment`,{method:'POST',body:JSON.stringify({catalogItemId:item.catalog_item_id,quantity:item.quantity,location:item.location,status:item.status,serialNumber:item.serial_number,notes:item.notes})});setNotice('השורה שוכפלה');onReload()}catch(error){setNotice(error.message)}}}><Copy size={14}/></button>}{user.role==='admin'&&<button onClick={()=>onDelete(item)}><Trash2 size={14}/></button>}</span></div>)}<button className="project-add-subitem" onClick={onAdd}><Plus size={15}/>הוסף שורה</button></div>}</div>})}</div></div>}</section>
}
