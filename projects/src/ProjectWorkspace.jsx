...
export function ProjectWorkspace({
  ...
  const addUpdate = async (e) => {
    ...
    if (!note.trim()) return;
    try {
      const result = await api(`/projects/${project.id}/updates`, {
        method: "POST",
    ...
    try {
      await api(`/projects/${project.id}/equipment`, {
        ...
        body: JSON.stringify({
          ...
          manualName:f.get("manualName"),projectSystemId:f.get("projectSystemId"),manufacturer:f.get("manufacturer"),model:f.get("model"),
  ...
  const uploadRecordFiles=async(files,title,category,relatedEntityType,relatedEntityId)=>{
    const pending=files.filter(file=>file instanceof File&&file.size);if(!pending.length)return;
    setUploadingFiles(true);
    try{for(let index=0;index<pending.length;index++){
      setRecordStatus(`מכין קובץ ${index+1} מתוך ${pending.length}: ${pending[index].name}`);
      const file=await prepareUploadImage(pending[index]);
      setRecordStatus(`מעלה קובץ ${index+1} מתוך ${pending.length}: ${pending[index].name}`);
      const body=new FormData();body.set('projectId',project.id);body.set('title',`${title} · ${pending[index].name}`);body.set('category',category);body.set('relatedEntityType',relatedEntityType);body.set('relatedEntityId',String(relatedEntityId));body.set('file',file);await api('/documents',{method:'POST',body});
    ...
    <div className="project-detail project-workspace">
      ...
      {["systems","team"].includes(tab) && (
        <div className="project-two-columns">
          {tab === "systems" && <details className="panel project-resource project-bom"><summary className="bom-summary"><span><strong>סיכום ציוד והתקדמות</strong><small><b>BOM</b> הוא כתב הכמויות של הפרויקט: מה הוזמן, כמה הותקן, כמה תוכנת ומה נשאר לביצוע — בחלוקה לפי מערכות.</small></span><span className="bom-summary-action"><span className="when-closed">הצגת הסיכום</span><span className="when-open">סגירת הסיכום</span><ChevronDown size={18}/></span></summary><div className="bom-content">{[...new Map(bom.map((item)=>[item.project_system_id||'none',{name:item.system_name||'ללא מערכת',color:item.system_color||'#6957df'}])).entries()].map(([systemId,system])=><div className="bom-system" key={systemId}><header style={{borderInlineStartColor:system.color}}><strong>{system.name}</strong></header><div className="bom-head"><span>פריט</span><span>הוזמן</span><span>הותקן</span><span>תוכנת</span><span>יתרה</span></div>{bom.filter((item)=>String(item.project_system_id||'none')===String(systemId)).map((item)=><article key={item.id}><div><strong>{item.name}</strong><small>{item.priority_sku||item.code}</small></div><b>{item.ordered}</b><input type="number" min="0" max={item.ordered} step="1" value={item.installed} disabled={!canEdit} onChange={(event)=>setBom((current)=>current.map((row)=>row.id===item.id?{...row,installed:Number(event.target.value),remaining:Math.max(0,row.ordered-Number(event.target.value))}:row))} onBlur={(event)=>updateBom(item,{installed:Number(event.target.value),programmed:item.programmed})}/><input type="number" min="0" max={item.installed} step="1" value={item.programmed} disabled={!canEdit} onChange={(event)=>setBom((current)=>current.map((row)=>row.id===item.id?{...row,programmed:Number(event.target.value)}:row))} onBlur={(event)=>updateBom(item,{installed:item.installed,programmed:Number(event.target.value)})}/><strong className={item.remaining?'remaining':''}>{item.remaining}</strong></article>)}</div>)}</div></details>}
      ...
      {modal === "equipment" && (
        <Modal title="הוספת ציוד לפרויקט" onClose={() => setModal("")}>
          <form className="work-form" onSubmit={addEquipment}>
            <label className="wide check-label"><input type="checkbox" checked={manualEquipment} onChange={e=>setManualEquipment(e.target.checked)}/>הוספת פריט ידני חדש (יישמר גם בקטלוג)</label>
            {manualEquipment?<><label className="wide">שם הפריט<input name="manualName" required maxLength={200}/></label><label>יצרן<input name="manufacturer"/></label><label>דגם<input name="model"/></label></>:<CatalogItemSelect items={reference.equipment}/>}
            <label className="wide">מערכת / ITEM<select name="projectSystemId" required={manualEquipment}><option value="">לפי הקטלוג / ללא מערכת</option>{reference.equipment.filter(x=>x.active&&x.itemType==='system').map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
            <label>
              כמות
              <input
                name="quantity"
      ...
      {recordStatus&&<ModalPortal><div className="record-upload-shield" role="status" aria-live="assertive" aria-atomic="true"><div className="record-upload-overlay"><Upload className="spin" size={22}/><strong>{uploadingFiles?'מעלה קבצים…':'שומר ביקורת / סיכום…'}</strong><span>{recordStatus}</span><small>נא להמתין עד לסיום השמירה וההעלאה.</small></div></div></ModalPortal>}
...
function LegacyProjectSystemsBoard({items,columns,canEdit,canManage,user,api,projectId,onAdd,onReload,onDelete,setNotice}) {
  ...
  const subitemGrid=`minmax(280px,2.5fr) minmax(92px,.8fr) minmax(82px,.7fr) minmax(105px,.9fr) 68px 68px minmax(98px,.85fr) ${columns.map(()=>'minmax(92px,.75fr)').join(' ')} 104px`;
  return <section className="project-systems-board panel">
    <header className="project-systems-board-title">
      <div className="systems-board-heading"><span className="systems-board-icon"><PanelsTopLeft size={19}/></span><span><h3>מערכות ורכיבים</h3><small>ניהול ציוד לפי מערכות, מיקומים והתקדמות התקנה</small></span></div>
      <div className="systems-board-actions">{canManage&&<div className="add-column-control"><input value={newColumn} onChange={event=>setNewColumn(event.target.value)} onKeyDown={event=>event.key==='Enter'&&addColumn()} placeholder="שם עמודה חדשה"/><button type="button" onClick={addColumn} disabled={!newColumn.trim()}><Plus size={15}/>הוספת עמודה</button></div>}{canEdit&&<button type="button" className="primary-button" onClick={onAdd}><Plus size={16}/>הוספת רכיב</button>}</div>
    </header>
    {!groups.length?<div className="inline-empty">טרם שויכו מערכות ורכיבים לפרויקט.</div>:<div className="project-systems-scroll"><div className="project-systems-table" style={{'--project-custom-columns':columns.length}}>
      <div className="project-system-head"><span>מערכת / ITEM</span><span>קטגוריה</span><span>סה״כ</span><span>הותקן</span><span>יתרה</span><span>צבע</span></div>
      {groups.map(group=>{const total=group.items.reduce((sum,item)=>sum+Number(item.quantity_ordered??item.quantity??0),0);const installed=group.items.reduce((sum,item)=>sum+Number(item.quantity_installed||0),0);const remaining=Math.max(0,total-installed);const key=group.id||group.name;const open=expanded.has(key);return <article onDragOver={event=>event.preventDefault()} onDrop={()=>moveGroup(group.id)} className={`project-system-item ${open?'is-open':''}`} key={key} style={{'--system-color':group.color}}>
        <div className="project-system-row">
          <button type="button" className="system-expand" onClick={()=>toggle(key)} aria-label={open?'סגירת מערכת':'פתיחת מערכת'}>{open?<ChevronDown size={18}/>:<ChevronLeft size={18}/>}</button>
          <span className="system-title-cell"><i draggable={canEdit&&Boolean(group.id)} onDragStart={()=>setDraggedGroup(group.id)} title="גרירת המערכת"><GripVertical size={16}/></i><span className="system-identity"><input disabled={!canManage||!group.id} defaultValue={group.name} onBlur={event=>saveGroup(group,{title:event.target.value})}/><small>{group.items.length} רכיבים</small></span></span>
          <span className="system-category"><i style={{background:group.color}}/>{group.type}</span><b>{total}</b><b className="installed">{installed}</b><b className="remaining">{remaining}</b>
          <label className="color-control" title="בחירת צבע למערכת" style={{'--selected-color':group.color}}><Palette size={15}/><input disabled={!canManage||!group.id} type="color" value={group.color} onChange={event=>saveGroup(group,{color:event.target.value})}/></label>
        </div>
        {open&&<div className="project-subitems">
          <div className="project-subitem-head" style={{gridTemplateColumns:subitemGrid}}><span>רכיב</span><span>מיקום</span><span>תיוג</span><span>מק״ט</span><span>כמות</span><span>הותקן</span><span>סטטוס</span>{columns.map((column,index)=><span key={column.column_key} draggable onDragStart={event=>{event.stopPropagation();event.dataTransfer.setData('text/plain',String(index))}} onDragOver={event=>event.preventDefault()} onDrop={event=>{event.stopPropagation();moveColumn(Number(event.dataTransfer.getData('text/plain')),index)}}><GripVertical size={12}/><b>{column.label}</b>{canManage&&<button type="button" title="מחיקת עמודה" onClick={async()=>{await api(`/projects/${projectId}/system-columns/${column.id}`,{method:'DELETE'});onReload()}}>×</button>}</span>)}<span>פעולות</span></div>
          {group.items.map(item=><div onDragOver={event=>event.preventDefault()} onDrop={event=>{event.stopPropagation();moveRow(group,item.id)}} className="project-subitem-row" key={item.id} style={{'--row-color':item.row_color||group.color,gridTemplateColumns:subitemGrid}}>
            <span className="subitem-name"><i draggable={canEdit} onDragStart={event=>{event.stopPropagation();setDraggedRow(item.id)}} title="גרירת הרכיב"><GripVertical size={14}/></i><strong title={item.name}>{item.name}</strong></span>
            <label className="subitem-field"><small>מיקום</small><input aria-label="מיקום" disabled={!canEdit} defaultValue={item.location||''} placeholder="הוספת מיקום" onBlur={event=>save(item,{location:event.target.value})}/></label><label className="subitem-field"><small>תיוג</small><input aria-label="תיוג" disabled={!canEdit} defaultValue={item.tag||''} placeholder="הוספת תיוג" onBlur={event=>save(item,{tag:event.target.value})}/></label><span className="sku-cell" title={item.priority_sku||item.code||''}><small>מק״ט</small>{item.priority_sku||item.code||'—'}</span>
...
function ExecutionMedia({files,entityType,entityId,apiRoot,onPreview}){
  const linked=(files||[]).filter(file=>file.related_entity_type===entityType&&String(file.related_entity_id)===String(entityId));
  if(!linked.length)return null;
  return <section className="execution-media"><h3>תמונות ומסמכים <small>{linked.length}</small></h3><div>{linked.map(file=><button type="button" key={file.id} onClick={()=>onPreview(file)}>{file.mime_type?.startsWith('image/')?<img src={`${apiRoot}/documents/${file.id}/preview`} alt={file.title||file.original_name} loading="lazy"/>:<FileText size={25}/>}<span>{file.title||file.original_name}</span></button>)}</div></section>;
}
...
function ProjectHoursPanel({project,entries,professionals,api,setNotice,onDone,canEdit}){
  ...
  return <section className="project-hours-page">
    ...
    <div className="project-hours-grid"><section className="panel"><h3>התפלגות שעות לפי פעילות</h3><div className="project-hours-chart" dir="ltr"><ResponsiveContainer width="100%" height={300}><BarChart data={totals} layout="vertical" margin={{top:8,right:18,bottom:4,left:86}}><CartesianGrid strokeDasharray="3 3" horizontal={false}/><XAxis type="number" allowDecimals={false} domain={[0,'auto']}/><YAxis type="category" dataKey="label" width={78} tickLine={false} axisLine={false} tick={{fill:'#626a7d',fontSize:12}}/><Tooltip formatter={(value)=>[`${value} שעות`,'בפועל']} labelFormatter={(label)=>String(label)} contentStyle={{direction:'rtl',textAlign:'right'}}/><Bar dataKey="hours" fill="#6957df" radius={[0,8,8,0]} minPointSize={2} isAnimationActive/></BarChart></ResponsiveContainer></div></section><section className="panel time-entry-list"><h3>דיווחים אחרונים</h3>{entries.slice(0,12).map(item=><article key={item.id}><i/><div><strong>{timeActivityLabels[item.activity_type]||item.activity_type}</strong><small>{item.professional_name||item.user_name||'משתמש'} · {dateText(item.work_date)}</small></div><b>{Number(item.hours)} ש׳</b>{canEdit&&<span className="time-entry-actions"><button type="button" title="עריכת דיווח" onClick={()=>{setEditing(item);setOpen(true)}}><Pencil size={14}/></button><button type="button" title="מחיקת דיווח" onClick={()=>remove(item)}><Trash2 size={14}/></button></span>}</article>)}{!entries.length&&<div className="inline-empty">טרם דווחו שעות לפרויקט.</div>}</section></div>
    {open&&<Modal title={editing?'עריכת דיווח שעות':'דיווח שעות עבודה'} onClose={()=>{setOpen(false);setEditing(null)}}><form className="work-form" onSubmit={submit}><label>סוג פעילות<select name="activityType" required defaultValue={editing?.activity_type||'planning'}>{Object.entries(timeActivityLabels).map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label><label>תאריך<DateInput type="date" name="workDate" required defaultValue={editing?localDateValue(editing.work_date):localDateValue()}/></label><label>מספר שעות<input type="number" name="hours" required min="0.25" max="24" step="0.25" autoFocus defaultValue={editing?.hours||''}/></label><label>מבצע<select name="professionalId" defaultValue={editing?.professional_id||''}><option value="">המשתמש המדווח</option>{professionals.filter(item=>item.active!==false).map(item=><option value={item.id} key={item.id}>{item.displayName}</option>)}</select></label><label className="wide">הערות<textarea name="notes" defaultValue={editing?.notes||''}/></label><div className="form-actions wide"><button type="button" className="secondary-button" onClick={()=>{setOpen(false);setEditing(null)}}>ביטול</button><button className="ops-primary">{editing?'שמירת שינויים':'שמירת דיווח'}</button></div></form></Modal>}
  </section>;
}
