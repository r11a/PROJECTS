...
function ProjectSystemsBoard({items,columns,canEdit,canManage,user,api,projectId,onAdd,onReload,onDelete,setNotice}) {
  ...
  const saveGroup=async(group,patch)=>{if(!group.id)return;try{await api(`/projects/${projectId}/system-board/${group.id}`,{method:'PATCH',body:JSON.stringify({title:patch.title??group.name,color:patch.color??group.color,sortOrder:group.sort})});await onReload()}catch(error){setNotice(error.message)}};
      ...
      {groups.map(group=>{const total=group.items.reduce((sum,item)=>sum+Number(item.quantity_ordered??item.quantity??0),0);const installed=group.items.reduce((sum,item)=>sum+Number(item.quantity_installed||0),0);const remaining=Math.max(0,total-installed);const key=group.id||group.name;const open=expanded.has(key);return <article onDragOver={event=>event.preventDefault()} onDrop={()=>moveGroup(group.id)} className={`project-system-item ${open?'is-open':''}`} key={key} style={{'--system-color':group.color}}>
        ...
        {open&&<div className="project-subitems">
          ...
          {group.items.map(item=><div onDragOver={event=>event.preventDefault()} onDrop={event=>{event.stopPropagation();moveRow(group,item.id)}} className="project-subitem-row" key={item.id} style={{'--row-color':item.row_color||group.color,gridTemplateColumns:subitemGrid}}>
            ...
            <input aria-label="מיקום" disabled={!canEdit} defaultValue={item.location||''} placeholder="הוספת מיקום" onBlur={event=>save(item,{location:event.target.value})}/><input aria-label="תיוג" disabled={!canEdit} defaultValue={item.tag||''} placeholder="הוספת תיוג" onBlur={event=>save(item,{tag:event.target.value})}/><span className="sku-cell" title={item.priority_sku||item.code||''}>{item.priority_sku||item.code||'—'}</span>
            <input aria-label="כמות" disabled={!canEdit} type="number" min="0" step="1" defaultValue={Number(item.quantity_ordered??item.quantity??0)} onBlur={event=>save(item,{quantity:Number(event.target.value)})}/><input aria-label="כמות שהותקנה" disabled={!canEdit} type="number" min="0" step="1" max={Number(item.quantity_ordered??item.quantity??0)} defaultValue={Number(item.quantity_installed||0)} onBlur={event=>save(item,{quantityInstalled:Number(event.target.value),status:Number(event.target.value)>=Number(item.quantity_ordered??item.quantity??0)?'installed':item.status})}/>
            <select aria-label="סטטוס" disabled={!canEdit} className={`monday-status ${item.status||'waiting'}`} value={item.status==='planned'?'waiting':item.status||'waiting'} onChange={event=>save(item,{status:event.target.value,quantityInstalled:event.target.value==='installed'?Number(item.quantity_ordered??item.quantity??0):Number(item.quantity_installed||0)})}><option value="waiting">ממתין</option><option value="in_progress">בביצוע</option><option value="installed">הותקן</option></select>
            {columns.map(column=><input disabled={!canEdit} key={column.column_key} defaultValue={item.custom_values?.[column.column_key]||''} onBlur={event=>save(item,{customValues:{...(item.custom_values||{}),[column.column_key]:event.target.value}})}/>)}
            <span className="project-subitem-actions"><label className="color-control compact" title="צבע שורה" style={{'--selected-color':item.row_color||group.color}}><Palette size={13}/><input disabled={!canEdit} type="color" value={item.row_color||group.color} onChange={event=>save(item,{rowColor:event.target.value})}/></label>{canEdit&&<button type="button" title="שכפול" onClick={async()=>{try{await api(`/projects/${projectId}/equipment`,{method:'POST',body:JSON.stringify({catalogItemId:item.catalog_item_id,quantity:item.quantity,location:item.location,status:item.status,serialNumber:item.serial_number,notes:item.notes})});setNotice('השורה שוכפלה');onReload()}catch(error){setNotice(error.message)}}}><Copy size={14}/></button>}{user.role==='admin'&&<button type="button" title="מחיקה" className="danger" onClick={()=>onDelete(item)}><Trash2 size={14}/></button>}</span>
          </div>)}
