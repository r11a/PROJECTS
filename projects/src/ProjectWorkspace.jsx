      ...
      {groups.map(group=>{const total=group.items.reduce((sum,item)=>sum+Number(item.quantity_ordered??item.quantity??0),0);const installed=group.items.reduce((sum,item)=>sum+Number(item.quantity_installed||0),0);const remaining=Math.max(0,total-installed);const key=group.id||group.name;const open=expanded.has(key);return <article onDragOver={event=>event.preventDefault()} onDrop={()=>moveGroup(group.id)} className={`project-system-item ${open?'is-open':''}`} key={key} style={{'--system-color':group.color}}>
        ...
        {open&&<div className="project-subitems">
          ...
          {group.items.map(item=><div onDragOver={event=>event.preventDefault()} onDrop={event=>{event.stopPropagation();moveRow(group,item.id)}} className="project-subitem-row" key={item.id} style={{'--row-color':item.row_color||group.color,gridTemplateColumns:subitemGrid}}>
            ...
            <details className="mobile-subitem-menu"><summary aria-label="פעולות רכיב"><MoreHorizontal size={20}/></summary><div><label className="mobile-menu-action color" style={{'--selected-color':item.row_color||group.color}}><Palette size={16}/><span>שינוי צבע</span><input disabled={!canEdit} type="color" value={item.row_color||group.color} onChange={event=>save(item,{rowColor:event.target.value})}/></label>{canEdit&&item.row_color&&<button type="button" onClick={()=>save(item,{rowColor:''})}><RotateCcw size={16}/><span>צבע המערכת</span></button>}{canEdit&&<button type="button" onClick={async()=>{try{await api(`/projects/${projectId}/equipment`,{method:'POST',body:JSON.stringify({catalogItemId:item.catalog_item_id,quantity:item.quantity,location:item.location,status:item.status,serialNumber:item.serial_number,notes:item.notes})});setNotice('השורה שוכפלה');onReload()}catch(error){setNotice(error.message)}}}><Copy size={16}/><span>שכפול</span></button>}{user.role==='admin'&&<button type="button" className="danger" onClick={()=>onDelete(item)}><Trash2 size={16}/><span>מחיקה</span></button>}</div></details>
          </div>)}
