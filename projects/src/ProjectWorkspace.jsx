...
function ProjectHoursPanel({project,entries,professionals,api,setNotice,onDone,canEdit,openRequest=0}){
  ...
  const submit=async(event)=>{event.preventDefault();const data=new FormData(event.currentTarget);try{await api(`/projects/${project.id}/time-entries${editing?`/${editing.id}`:''}`,{method:editing?'PATCH':'POST',body:JSON.stringify({activityType:data.get('activityType'),workDate:data.get('workDate'),hours:data.get('hours'),professionalId:data.get('professionalId')||null,notes:data.get('notes')})});setOpen(false);setEditing(null);setNotice(editing?'השינוי בדיווח השעות נשמר בהצלחה':'דיווח השעות נשמר בהצלחה ובנק השעות עודכן');await onDone()}catch(error){setNotice(error.message)}};
