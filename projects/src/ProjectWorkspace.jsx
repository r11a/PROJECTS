      ...
      {modal === "equipment" && (
        <Modal title="הוספת ציוד לפרויקט" onClose={() => setModal("")}>
          <form className="work-form" onSubmit={addEquipment}>
            ...
            {manualEquipment?<><label className="wide">שם הפריט<input name="manualName" required maxLength={200}/></label><label>יצרן<input name="manufacturer"/></label><label>דגם<input name="model"/></label></>:<><label className="wide">חיפוש פריט קטלוג<input type="search" value={equipmentSearch} onChange={e=>setEquipmentSearch(e.target.value)} placeholder="שם, מק״ט, יצרן או דגם"/></label><label className="wide">פריט קטלוג<select name="catalogItemId" required><option value="">בחירה מהקטלוג</option>{reference.equipment.filter(x=>x.active&&`${x.name} ${x.code||''} ${x.prioritySku||''} ${x.manufacturer||''} ${x.model||''}`.toLocaleLowerCase('he').includes(equipmentSearch.trim().toLocaleLowerCase('he'))).map(x=><option key={x.id} value={x.id}>{x.name} {x.manufacturer} {x.model}</option>)}</select></label></>}
