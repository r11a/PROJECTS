        ...
        <Modal title="אשף הפקת דוח PDF" subtitle="בחירת תוכן, פרויקט ויעד השמירה" className="report-modal" onClose={() => setWizardOpen(false)}>
          <div className="report-wizard">
            ...
            <div className="pdf-report-sheet" ref={reportRef} dir="rtl">
              ...
              {reportType==='project'&&projectReport&&<p className="report-project-category">סיווג פרויקט: {projectReport.project.projectCategory==='other'?(projectReport.project.projectCategoryCustom||'אחר'):'בית חכם'}</p>}
              {reportType!=='project'&&data.projectCategories?.length>0&&<p className="report-project-category">תמהיל פרויקטים: {data.projectCategories.map(item=>`${item.category==='smart_home'?'בית חכם':item.custom_name} — ${item.count}`).join(' · ')}</p>}
