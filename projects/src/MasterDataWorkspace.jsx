...
function EquipmentTree({ items, apiRoot, api, refresh, setNotice, user, onCreate, onEdit, onDelete, onDuplicate }) {
  ...
  return <div className="equipment-board">
    ...
    {categories.map(category=>{const color=catalogColor(category);const systems=items.filter(item=>item.itemType==='system'&&String(item.parentId)===String(category.id)),categoryOpen=expandedCategories.has(category.id);return <section className={`equipment-board-group panel ${categoryOpen?'category-open':'category-collapsed'}`} key={category.id} style={{'--category-color':color}}><header><button type="button" className="equipment-category-toggle" aria-expanded={categoryOpen} onClick={()=>setExpandedCategories(current=>{const next=new Set(current);next.has(category.id)?next.delete(category.id):next.add(category.id);return next})}><span className="catalog-category-icon" style={{background:color}}>{category.iconImageStoredName?<img src={`${apiRoot}/equipment-catalog/${category.id}/icon`} alt=""/>:<Boxes size={20}/>}</span><span><strong>{category.name}</strong><small>{systems.length} מערכות · לחיצה {categoryOpen?'לצמצום':'להרחבה'}</small></span>{categoryOpen?<ChevronDown size={18}/>:<ChevronLeft size={18}/>}</button>{editable&&<span className="equipment-category-actions"><button onClick={()=>onCreate('system',category.id)} title="הוספת תת־קטגוריה או מערכת"><Plus size={15}/><span>מערכת</span></button><button onClick={()=>onEdit(category)} title="עריכת סוג מערכת"><Pencil size={15}/><span>עריכה</span></button></span>}</header>
