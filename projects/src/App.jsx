...
function App() {
  ...
  const [projects, setProjects] = useState(() => {
    try { return JSON.parse(localStorage.getItem('nexa-projects')) || seedProjects; } catch { return seedProjects; }
  });
  ...
  useEffect(() => { localStorage.setItem('nexa-projects', JSON.stringify(projects)); }, [projects]);
  useEffect(() => {
    if (!notice) return;
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        <div className="brand"><div className="brand-mark"><Sparkles size={22} /></div><div><strong>NEXA</strong><span>PROJECTS</span></div></div>
