...
import {
  Activity, AlertTriangle, Archive, ArrowLeft, Bell, Building2, CalendarDays, Check, CheckCircle2,
...
const actionNamesForDashboard = { create: 'יצר רשומה', update: 'עדכן רשומה', delete: 'מחק רשומה', archive: 'העביר לארכיון', restore: 'שחזר מהארכיון', upload: 'העלה קובץ', login: 'נכנס למערכת', snooze: 'דחה התראה', backup: 'יצר גיבוי' };
...
function App() {
  ...
  const updateProject = async (id, patch) => {
    try {
      ...
      if (patch.clientId || patch.newClient || patch.clientName !== undefined) await loadReferenceData();
  ...
  const archiveProject = async (id, archived) => {
    try {
      await api(`/projects/${encodeURIComponent(id)}/archive`, { method: 'PATCH', body: JSON.stringify({ archived }) });
      const result = await api('/projects');
      setProjects(result.projects);
      setSelectedProject(null);
      setPage('projects');
      setNotice(archived ? 'הפרויקט הועבר לארכיון' : 'הפרויקט שוחזר לרשימת הפרויקטים הפעילים');
      return true;
    ...
    <div className={`app-shell${darkMode ? ' theme-dark' : ''}`}>
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-footer">
          ...
          <div className="sidebar-version"><span>PROJECTS</span><b>v{packageJson.version}</b></div>
      ...
      <main className="main-area">
        ...
        <div className="page-content">
          ...
          {page === 'project' && selectedProject && <ProjectWorkspace project={projects.find((p) => p.id === selectedProject.id) || selectedProject} updateProject={updateProject} archiveProject={archiveProject} api={api} apiRoot={apiRoot} user={user} projects={projects} clients={clientOptions} professionals={professionals} stageOptions={stageOptions} setNotice={setNotice} setPage={setPage} />}
        </div>
      </main>
      {newProjectOpen && <NewProjectModal api={api} onClose={() => setNewProjectOpen(false)} onCreate={createProject} professionals={professionals} clients={clientOptions} stageOptions={stageOptions} equipment={equipmentCatalog} />}
...
function ProjectsPage({ projects, search, setSearch, stageFilter, setStageFilter, openProject, api, setNotice }) {
  const [view, setView] = useState('table');
  const [filtersOpen, setFiltersOpen] = useState(false);
  const [showArchived, setShowArchived] = useState(false); const [archivedProjects, setArchivedProjects] = useState([]); const [archiveLoading, setArchiveLoading] = useState(false);
  ...
  const switchArchive = async (archived) => {
    setShowArchived(archived); setStageFilter('all');
    if (!archived) return;
    setArchiveLoading(true);
    try { const result = await api('/projects?scope=archived'); setArchivedProjects(result.projects); }
    catch (error) { setNotice(error.message); } finally { setArchiveLoading(false); }
  };
  const sourceProjects = showArchived ? archivedProjects.filter((project) => `${project.name} ${project.client} ${project.location} ${project.id}`.toLowerCase().includes(search.toLowerCase()) && (stageFilter === 'all' || project.stage === stageFilter)) : projects;
  const visibleProjects = sourceProjects.filter((project) => (!manager || project.manager === manager) && (!priority || project.priority === priority) && (!flagged || project.flag));
  ...
  return <div className="section-page">
    ...
    <div className="toolbar panel projects-filter-toolbar"><label className="table-search"><Search size={18} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="חיפוש פרויקט, לקוח או מזהה..." /></label><label className="stage-filter-select" style={{'--stage-color':stageFilter==='all'?'#6957df':stageMeta[stageFilter]?.color||'#6957df'}}><span><i/>שלב</span><select value={stageFilter} onChange={event=>setStageFilter(event.target.value)}><option value="all">כל השלבים · {sourceProjects.length}</option>{Object.entries(stageMeta).map(([key,meta])=><option key={key} value={key}>{meta.label} · {sourceProjects.filter(project=>project.stage===key).length}</option>)}</select><ChevronDown size={16}/></label><button className={`filter-button ${filtersOpen?'active':''}`} onClick={()=>setFiltersOpen(!filtersOpen)}><SlidersHorizontal size={17} />מסננים</button></div>
...
function NewProjectModal({ api, onClose, onCreate, professionals, clients, stageOptions, equipment }) {
  ...
  const [form, setForm] = useState({ name: '', clientMode:'existing', clientId: '', clientName:'', clientPhone:'', clientEmail:'', clientAddress:'', clientCity:'', location: '', managerId: managers[0]?.id || '', stage: stageOptions[0]?.metadata?.key || 'planning', value: '', startDate:new Date().toISOString().slice(0,10), targetDate:'', selectedEquipment:{} });
  const submit = (event) => {
    event.preventDefault();
    ...
    const newClient=form.clientMode==='new'?{name:form.clientName,address:form.clientAddress,phone:form.clientPhone,email:form.clientEmail,city:form.clientCity}:undefined;
    onCreate({ name: form.name, clientId: client?.id || null, newClient, location: form.location || client?.city || form.clientCity || '', address: client?.address || form.clientAddress || form.location, lat: 32.08, lng: 34.82, stage: form.stage, progress: 0, managerId: manager?.id || null, manager: manager?.displayName || '', ownerInitials: manager?.displayName?.slice(0,2) || '', value: Number(form.value) || 0, paid: 0, due: form.targetDate ? new Date(form.targetDate).toLocaleDateString('he-IL') : '', priority: 'normal', flag: '', systems: [], equipmentItems, nextMilestone: 'פגישת אפיון ראשונית', phone: client?.phone || form.clientPhone || '', email: client?.email || form.clientEmail || '', health: 100, tasksDone: 0, tasksTotal: 0 });
  };
  const categories=equipment.filter(item=>item.itemType==='system_type'&&item.active);
  if(step===1)return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal project-wizard" onMouseDown={(event)=>event.stopPropagation()}><div className="modal-head"><div><span>אשף פרויקט חדש · שלב 1 מתוך 3</span><h2>לקוח וזהות הפרויקט</h2></div><button onClick={onClose}><X/></button></div><div className="wizard-progress"><i style={{width:'33.333%'}}/></div><form onSubmit={submit}><label>שם הפרויקט<input autoFocus required value={form.name} onChange={(event)=>setForm({...form,name:event.target.value})} placeholder="לדוגמה: וילה משפחת ישראלי"/></label><div className="client-mode-switch"><button type="button" className={form.clientMode==='existing'?'active':''} onClick={()=>setForm({...form,clientMode:'existing'})}>לקוח קיים</button><button type="button" className={form.clientMode==='new'?'active':''} onClick={()=>setForm({...form,clientMode:'new',clientId:''})}>לקוח חדש</button></div>{form.clientMode==='existing'?<div className="form-row"><label>לקוח<select required value={form.clientId} onChange={(event)=>setForm({...form,clientId:event.target.value})}><option value="">בחירת לקוח מהמאגר</option>{clients.map((client)=><option key={client.id} value={client.id}>{client.name} · {client.address}</option>)}</select></label><label>עיר / מיקום<input value={form.location} onChange={(event)=>setForm({...form,location:event.target.value})} placeholder="נלקח מכתובת הלקוח אם ריק"/></label></div>:<div className="new-client-fields"><p>הלקוח ייווצר אוטומטית במאגר ויקושר לפרויקט.</p><div className="form-row"><label>שם לקוח<input required value={form.clientName} onChange={(event)=>setForm({...form,clientName:event.target.value})}/></label><label>טלפון<input required value={form.clientPhone} onChange={(event)=>setForm({...form,clientPhone:event.target.value})}/></label></div><AddressAutocomplete api={api} required value={form.clientAddress} onChange={(clientAddress)=>setForm((current)=>({...current,clientAddress}))} onSelect={(item)=>setForm((current)=>({...current,clientAddress:item.address,clientCity:item.city||current.clientCity,location:item.city||current.location,lat:item.lat,lng:item.lng}))}/><label>עיר<input value={form.clientCity} onChange={(event)=>setForm({...form,clientCity:event.target.value})}/></label><label>דוא״ל<input type="email" value={form.clientEmail} onChange={(event)=>setForm({...form,clientEmail:event.target.value})}/></label></div>}<div className="modal-actions"><button type="button" onClick={onClose}>ביטול</button><button className="primary-button" type="submit">המשך <ArrowLeft size={16}/></button></div></form></div></div>;
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal project-wizard" onMouseDown={(e) => e.stopPropagation()}><div className="modal-head"><div><span>אשף פרויקט חדש · שלב {step} מתוך 3</span><h2>{step===1?'לקוח וזהות הפרויקט':step===2?'ניהול ולוחות זמנים':'מערכות וסקירה'}</h2></div><button onClick={onClose}><X /></button></div><div className="wizard-progress"><i style={{width:`${step/3*100}%`}}/></div><form onSubmit={submit}>{step===1&&<><label>שם הפרויקט<input autoFocus required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="לדוגמה: וילה משפחת ישראלי" /></label><div className="client-mode-switch"><button type="button" className={form.clientMode==='existing'?'active':''} onClick={()=>setForm({...form,clientMode:'existing'})}>לקוח קיים</button><button type="button" className={form.clientMode==='new'?'active':''} onClick={()=>setForm({...form,clientMode:'new',clientId:''})}>לקוח חדש</button></div>{form.clientMode==='existing'?<div className="form-row"><label>לקוח<select required value={form.clientId} onChange={(e) => setForm({ ...form, clientId: e.target.value })}><option value="">בחירת לקוח מהמאגר</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.name} · {client.address}</option>)}</select></label><label>עיר / מיקום<input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="נלקח מכתובת הלקוח אם ריק" /></label></div>:<div className="new-client-fields"><p>הלקוח ייווצר אוטומטית במאגר ויקושר לפרויקט.</p><div className="form-row"><label>שם לקוח<input required value={form.clientName} onChange={e=>setForm({...form,clientName:e.target.value})}/></label><label>טלפון<input required value={form.clientPhone} onChange={e=>setForm({...form,clientPhone:e.target.value})}/></label></div><div className="form-row"><label>כתובת<input required value={form.clientAddress} onChange={e=>setForm({...form,clientAddress:e.target.value})}/></label><label>עיר<input value={form.clientCity} onChange={e=>setForm({...form,clientCity:e.target.value})}/></label></div><label>דוא״ל<input type="email" value={form.clientEmail} onChange={e=>setForm({...form,clientEmail:e.target.value})}/></label></div>}</>}{step===2&&<><div className="form-row"><label>מנהל פרויקט<select value={form.managerId} onChange={(e) => setForm({ ...form, managerId: e.target.value })}><option value="">ללא מנהל</option>{managers.map((manager) => <option key={manager.id} value={manager.id}>{manager.displayName}</option>)}</select></label><label>שלב התחלתי<select value={form.stage} onChange={(e) => setForm({ ...form, stage:e.target.value })}>{(stageOptions.length ? stageOptions.map((item) => [item.metadata?.key || item.name,item.name]) : [['planning','תכנון']]).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><div className="form-row"><label>תאריך התחלה<input type="date" required value={form.startDate} onChange={e=>setForm({...form,startDate:e.target.value})}/></label><label>יעד מסירה<input type="date" min={form.startDate} value={form.targetDate} onChange={e=>setForm({...form,targetDate:e.target.value})}/></label></div><label>שווי משוער<input type="number" min="0" value={form.value} onChange={(e) => setForm({ ...form, value: e.target.value })} placeholder="₪ 0" /></label></>}{step===3&&<><p className="wizard-help">בחרו מערכות ראשוניות וכמות. אפשר להוסיף, לשנות או להסיר בהמשך מתוך הפרויקט.</p><div className="wizard-systems">{categories.map(category=><section key={category.id}><h3>{category.name}</h3>{equipment.filter(item=>String(item.parentId)===String(category.id)&&item.active).map(item=><label key={item.id}><span>{item.name}</span><input type="number" min="0" placeholder="0" value={form.selectedEquipment[item.id]||''} onChange={e=>setForm({...form,selectedEquipment:{...form.selectedEquipment,[item.id]:e.target.value}})}/></label>)}</section>)}</div></>}<div className="modal-actions"><button type="button" onClick={()=>step===1?onClose():setStep(step-1)}>{step===1?'ביטול':'חזרה'}</button><button className="primary-button" type="submit">{step===3?'יצירת פרויקט':'המשך'} <ArrowLeft size={16} /></button></div></form></div></div>;
}
